...
def clean_response(text):
    # (keep your existing clean_response function exactly as is)
...
def respond(message, history):
    ...
    if not AGENT_AVAILABLE:
        yield "ERROR: The Biomni agent is not available for an unknown reason.", None
        return
    ...
    if agent_instance is None:
        try:
            yield "Initializing the Biomni agent and downloading the data lake... This may take a few minutes.", None
            agent_instance = A1(
                llm='gpt-4.1-mini',
            ...
            yield "Agent initialized! Processing your request...", None
        except Exception as e:
    ...
    try:
        ...
        for chunk in agent_instance.go_stream(full_input):
            ...
            if "output" in chunk and isinstance(chunk["output"], str):
                ...
                yield loading_frames[step_idx % 3], None
                step_idx += 1
        ...
        pdf_path = None
        if any(keyword in message.lower() for keyword in ["pdf", "report", "save this", "download this", "generate a report"]):
            try:
                sections = []
                ...
                pdf_path = generate_pdf("Biomni Report", sections)
...
def main(host: str, port: int):
    ...
    with gr.Blocks(theme=theme, css=css_content) as iface:
        ...
        def chat_and_file(message, history):
            ...
            if final_file:
                return history, gr.update(value=final_file, visible=True)
